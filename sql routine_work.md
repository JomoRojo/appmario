create table public.routine_work (
  ...
  constraint routine_work_pkey primary key (id),
  constraint routine_work_closet_id_key unique (closet_id),
  constraint routine_work_closet_id_fkey foreign KEY (closet_id) references closets (id) on delete CASCADE,
...
create index IF not exists routine_work_closet_id_idx on public.routine_work using btree (closet_id) TABLESPACE pg_default;

create trigger routine_work_formality_score BEFORE INSERT
or
update on routine_work for EACH row
execute FUNCTION handle_work_formality_score ();

create trigger routine_work_sync_closet
after INSERT
or DELETE on routine_work for EACH row
execute FUNCTION handle_closet_routine_work ();

create trigger routine_work_updated_at BEFORE
update on routine_work for EACH row
